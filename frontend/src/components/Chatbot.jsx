...
// AHORA RECIBE EL PARÁMETRO "mode" (por defecto será 'admin')
const Chatbot = ({ mode = 'admin' }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      {/* ENCABEZADO DINÁMICO (Cambia según el modo) */}
      <div className={`px-6 py-4 border-b border-slate-200 flex items-center justify-between ${
        mode === 'admin' ? 'bg-slate-50' : 'bg-blue-600'
      }`}>
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg ${
            mode === 'admin' ? 'bg-blue-600 text-white' : 'bg-white/20 text-white'
          }`}>
            <Bot size={20} />
          </div>
          <div>
            <h2 className={`font-bold text-lg leading-tight ${
              mode === 'admin' ? 'text-slate-800' : 'text-white'
            }`}>
              {mode === 'admin' ? 'Terminal de Diagnóstico IA' : 'Asistente Virtual Innotrev'}
            </h2>
            <div className={`flex items-center gap-2 text-xs mt-0.5 ${
              mode === 'admin' ? 'text-slate-500' : 'text-blue-100'
            }`}>
              <span className="relative flex h-2 w-2">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                  mode === 'admin' ? 'bg-emerald-400' : 'bg-white'
                }`}></span>
                <span className={`relative inline-flex rounded-full h-2 w-2 ${
                  mode === 'admin' ? 'bg-emerald-500' : 'bg-white'
                }`}></span>
              </span>
      ...
      {/* ALERTAS DINÁMICAS (Solo se muestran si es modo Admin) */}
