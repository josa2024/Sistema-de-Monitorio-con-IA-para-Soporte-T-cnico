      ...
      {equiposInstalados.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEquipos.map(eq => {
            // 🔥 Variable que determina si el equipo está en falla/dañado
              ...
              <div key={eq.id} className={`bg-white border ${isDamaged ? 'border-red-200' : 'border-slate-200'} p-6 rounded-3xl shadow-sm hover:shadow-xl transition-all flex flex-col group relative overflow-hidden`}>
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {/* ETIQUETA DINÁMICA: "Protegido" o "Atención Requerida" */}
                    {isDamaged ? (
                      ...
                      <div key={lic.id} className="border-2 border-slate-100 rounded-3xl p-6 relative overflow-hidden">
                        ...
                        <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden flex flex-col md:flex-row items-stretch">
                          ...
                          <div className="p-4 md:w-2/3 bg-white flex items-center justify-center min-h-[150px]">
                            {lic.url_evidencia || selectedEqLicencias.url_evidencia ? (
                               <img src={`http://localhost:8000/${lic.url_evidencia || selectedEqLicencias.url_evidencia}`} alt="Evidencia Equipo" className="max-h-48 object-contain rounded-lg shadow-sm border border-slate-100" />
                            ) : (
