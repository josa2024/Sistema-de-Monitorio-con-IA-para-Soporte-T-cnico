      ...
      <AnimatePresence>
        {liveAlert && (
          <motion.div 
            ...
            className="fixed top-6 left-1/2 z-50 bg-blue-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3"
          >
                ...
                {ticketsList.length > 0 ? (
                  ticketsList.map(ticket => (
                    <motion.div 
                      ...
                      className="border border-slate-200 rounded-xl p-4 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer bg-white group"
                    >
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <div className="h-64 w-full">
              {chartData.length > 0 ? (
                ...
                <div className="h-full w-full flex items-center justify-center text-sm text-slate-400 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50">
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-white shadow-sm z-10">
                  <tr className="text-slate-500 text-xs uppercase tracking-wider border-b border-slate-200">
                    ...
                    <th className="py-4 px-6 font-semibold">Número de Serie</th>
                    <th className="py-4 px-6 font-semibold">Modelo</th>
                ...
                <tbody className="text-sm">
                  {equipmentList.length > 0 ? (
                    equipmentList.map((eq) => (
                      <motion.tr whileHover={{ backgroundColor: "#f8fafc" }} key={eq.id} className="border-b border-slate-100 transition-colors">
                        ...
                        <td className="py-4 px-6 text-slate-600">{eq.modelo}</td>
                        <td className="py-4 px-6"><span className={`px-2.5 py-1.5 rounded-full text-[10px] font-bold tracking-wide border ${getStatusColor(eq.status)}`}>{eq.status || 'SIN ESTADO'}</span></td>
                      </motion.tr>
                    ))
                  ) : (
        ...
        {selectedTicket && (
          ...
          >
            <motion.div 
              ...
              className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-200/50"
            >
              {/* ... El resto del modal se mantiene igual, la animación ya está aplicada al contenedor ... */}
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50 space-y-6">
                <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2"><AlertTriangle size={14}/> Reporte / Diagnóstico IA</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    {comments.map((comment, idx) => (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={idx} className="flex gap-3">
                        <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0 shadow-sm"><User size={14} /></div>
                        <div className="bg-white border border-slate-200 p-4 rounded-2xl rounded-tl-none text-sm text-slate-700 shadow-sm w-full">
                          <p>{comment.contenido}</p>
                          <span className="text-[10px] text-slate-400 mt-2 block font-medium">{new Date(comment.fecha_creacion).toLocaleString()}</span>
                        </div>
                      </motion.div>
                    ))}
                  </div>
              ...
              <div className="p-5 border-t border-slate-200 bg-white">
                {selectedTicket.status === 'ABIERTO' ? (
                  <div className="flex justify-end gap-3">
                    <button onClick={handleCloseModal} className="px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">Cancelar</button>
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleAssignTicket} disabled={isProcessing} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors shadow-md disabled:opacity-50">
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-3">
                      ...
                      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleAddComment} disabled={isProcessing || !newComment.trim()} className="bg-slate-800 hover:bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-colors shadow-md disabled:opacity-50">Enviar</motion.button>
                    </div>
                    <div className="flex justify-between items-center pt-3 border-t border-slate-100">
                      <span className="text-xs text-blue-600 font-medium bg-blue-50 px-3 py-1.5 rounded-lg">Estás trabajando en este ticket</span>
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleResolveTicket} disabled={isProcessing} className="bg-emerald-100 hover:bg-emerald-200 text-emerald-700 border border-emerald-300 px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors shadow-sm disabled:opacity-50">
